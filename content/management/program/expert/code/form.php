...
<?php
	...
	function insert($db){
		...
		by_db("
			insert into `$db`(`id`,`name`,`email`,`phone`,`theme`,`content`) 
			values(:id,:name,:email,:phone,:theme,:content) on duplicate key 
			update `name`=:name,`email`=:email,`phone`=:phone,`theme`=:theme,`content`=:content
		",$_POST);
